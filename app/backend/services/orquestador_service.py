...
co = cohere.ClientV2() # inicializo al cliente


def orquestador_llm(queryJSON: Message):
    
    query = queryJSON.message #desde la clase queryJSON me quiero quedar con el mensaje
    ...
    if response_primera_instancia != "rag":
        print(f"la respuesta del orquestador (en teoria distinto de rag) fue:{response_primera_instancia}")
    ...
    print(f"la respuesta del orquestador (en teoria rag) fue: {response_primera_instancia}")
    query_embedding = get_embeddings_query(query) # hace proceso de embedding de la query

    context = get_context(query_embedding) # hace retrive
    
    context_data = context["documents"] # contexto

    documents_list = [doc for sublist in context_data for doc in sublist]

    #"Segunda instancia, reranking

    lista_indices_rerank = rerank_responses(co,query,documents_list) 
    ...
    controlando = tercera_instancia_context(co,query,documentos_re_rank)
    print(f"controlando respuestaaaaaaaaaaaaaaaa: {controlando}")
    if controlando != "si":
