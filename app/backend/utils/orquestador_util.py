...
def primera_instancia(co,query: str):
    ...
    -si el usuario pregunta o consulta algo, esto debe pasar por la aplicacion a lo que debes contestar con una sola palabra que es "rag"
    ###
    Pregunta:
    {query}
    """
...
def rerank_responses(co,query,responses):
    ...
    lista_indices_reranking = [result.index for result in rerank_responses.results]
...
def tercera_instancia_context(co,query,documentos_re_rank):
    system_prompt = """
    Tu deber es solo clasificar el texto de acuerdo al contexto como base de informacion.
    """
    ...
    ####
    Instrucciones:
    -si la respuesta puede ser contestada de acuerdo al contexto tienes que responder con una sola palabra: "si"
    -si la respuesta no puede ser contestada de acuerdo al contexto tienes que responder con una sola palabra: "no"
    ###
    Contexto:
    {documentos_re_rank}
    ###
